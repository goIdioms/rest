...
func (r *TodoItemPostgres) Create(listId int, item models.TodoItem) (int, error) {
	...
	row := tx.QueryRow(createItemQuery, item.Title, item.Discription)
	err = row.Scan(&itemId)
	if err != nil {
		tx.Rollback()
		return 0, err
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]models.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.discription, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id
									INNER JOIN %s ul on ul.list_id = li.list_id WHERE li.list_id = $1 AND ul.user_id = $2`,
		todoItemsTable, listsItemTable, userListsTable)
...
func (r *TodoItemPostgres) Update(userId, itemId int, input models.UpdateItemInput) error {
	...
	query := fmt.Sprintf(`UPDATE %s ti SET %s FROM %s li, %s ul
		...
		todoItemsTable, setQuery, listsItemTable, userListsTable, argId, argId+1)
